...
# state and admin_state are stored at server module level; handlers expect admin_state dict to be passed
...
async def check_user_membership(user_id: int) -> Tuple[bool, List[str], Dict[str, str]]:
    ...
    for chat_ref in REQUIRED_CHATS:
        try:
            member = await safe_telegram_call(
                bot.get_chat_member(chat_id=chat_ref, user_id=user_id)
            )
    ...
    if user_id in admin_state:
        ...
        if action == "awaiting_add" and "|" in text:
            try:
                ...
                await db_execute(
                    "INSERT INTO buttons (name, callback_data, parent_id) VALUES ($1,$2,$3)",
                    name,
                    callback_data,
                    parent_id,
                )
                ...
                if bot:
                    await safe_telegram_call(
                        bot.send_message(chat_id=chat_id, text=f"تم إضافة الزر '{name}'")
                    )
                ...
                if bot:
                    await safe_telegram_call(
                        bot.send_message(chat_id=chat_id, text="خطأ في الإضافة")
                    )
            return
            ...
            try:
                ...
                if bot:
                    await safe_telegram_call(
                        bot.send_message(chat_id=chat_id, text="تم الحذف")
                    )
                ...
                if bot:
                    await safe_telegram_call(
                        bot.send_message(chat_id=chat_id, text="خطأ في الحذف")
                    )
            return
